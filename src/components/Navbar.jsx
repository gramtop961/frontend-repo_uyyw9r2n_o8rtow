...
export default function Navbar() {
  ...
  const links = [
    ...
    { href: 'https://calendly.com/flames-studio/intro-call', label: 'Book a call' },
  ...
  return (
    <div className="fixed top-0 inset-x-0 z-50">
      <motion.nav
        ...
        className={`mx-auto ${scrolled ? 'mt-3 max-w-md' : 'mt-4 max-w-6xl'} rounded-full border border-white/10 bg-black/60 backdrop-blur supports-[backdrop-filter]:bg-black/50 px-4 ${scrolled ? 'py-2' : 'py-3'}`}
      >
        <div className={`flex items-center ${scrolled ? 'justify-center' : 'justify-between'}`}>
          ...
          <a href="#" className="flex items-center gap-2 text-white">
            ...
            {!scrolled && <span className="font-semibold tracking-tight">Flames Studio</span>}
          </a>

          {/* Desktop links (hidden when shrunk) */}
          {!scrolled && (
            <div className="hidden md:flex items-center gap-6">
              ...
              <a
                href={links[2].href}
                className="inline-flex items-center gap-2 rounded-full border border-purple-500/40 bg-black/60 px-4 py-2 text-sm font-medium text-white/90 backdrop-blur hover:bg-purple-500/10 transition"
              >
                {links[2].label}
              </a>
              <a
          ...
          {/* Mobile toggle (only when not centered) */}
          {!scrolled && (
            <button
